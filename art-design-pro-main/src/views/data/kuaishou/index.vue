  ...
  <div class="page-container">
    <el-card shadow="never">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-bold">快手数据管理</h3>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑数据' : '新增数据'" width="500px">
      <el-form :model="formData" label-width="100px">
        ...
        <el-form-item label="时长">
          <el-input v-model="formData.duration" placeholder="如 5min" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import {
  fetchKuaishouList,
  fetchKuaishouAccounts,
  fetchAddKuaishou,
  fetchUpdateKuaishou,
  fetchDeleteKuaishou,
  fetchBatchDeleteKuaishou
} from '@/api/kuaishou'
...
const loadData = async () => {
  loading.value = true
  try { const res: any = await fetchKuaishouList(); tableData.value = res || [] } catch (e) { console.error(e) }
  loading.value = false
}

const loadAccounts = async () => {
  try { const res: any = await fetchKuaishouAccounts(); accounts.value = res || [] } catch (e) { console.error(e) }
}
...
const handleAdd = () => {
  ...
  formData.value = { accountName: '', recordDate: new Date().toISOString().split('T')[0], playCount: 0, likeCount: 0, commentCount: 0, shareCount: 0, exposure: 0, viewers: 0, duration: '0min' }
  dialogVisible.value = true
}

const handleEdit = (row: any) => {
  isEdit.value = true; editId.value = row.id
  formData.value = { accountName: row.accountName || '', recordDate: row.recordDate || new Date().toISOString().split('T')[0], playCount: row.playCount || 0, likeCount: row.likeCount || 0, commentCount: row.commentCount || 0, shareCount: row.shareCount || 0, exposure: row.exposure || 0, viewers: row.viewers || 0, duration: row.duration || '0min' }
  dialogVisible.value = true
}
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) { await fetchUpdateKuaishou(editId.value, submitData); ElMessage.success('更新成功') }
    else { await fetchAddKuaishou(submitData); ElMessage.success('添加成功') }
    dialogVisible.value = false; loadData()
  } catch (e) { console.error(e) }
}

const handleDelete = async (id: number) => {
  try { await fetchDeleteKuaishou(id); ElMessage.success('删除成功'); loadData() } catch (e) { console.error(e) }
}

const handleBatchDelete = async () => {
  try { await fetchBatchDeleteKuaishou(selectedIds.value); ElMessage.success('批量删除成功'); selectedIds.value = []; loadData() } catch (e) { console.error(e) }
}
