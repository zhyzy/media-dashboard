  ...
  <div class="page-container">
    <el-card shadow="never">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-bold">视频号数据管理</h3>
      ...
      <el-table :data="tableData" border stripe v-loading="loading" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="playCount" label="播放量" min-width="140">
          <template #default="{ row }">
            ...
            <span v-if="row.playCountGrowth > 0" class="ml-2 text-success">+{{ formatNumber(row.playCountGrowth) }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑数据' : '新增数据'" width="500px">
      <el-form :model="formData" label-width="100px">
        <el-form-item label="账号名称">
          <el-autocomplete v-model="formData.accountName" :fetch-suggestions="queryAccounts" placeholder="输入或选择账号" style="width: 100%" />
        </el-form-item>
...
import {
  fetchVideoAccountList,
  fetchVideoAccountAccounts,
  fetchAddVideoAccount,
  fetchUpdateVideoAccount,
  fetchDeleteVideoAccount,
  fetchBatchDeleteVideoAccount
} from '@/api/video-account'
...
const editId = ref(0)
const formData = ref({ accountName: '', recordDate: new Date().toISOString().split('T')[0], playCount: 0, likeCount: 0, commentCount: 0, shareCount: 0 })
...
const loadData = async () => {
  loading.value = true
  try { const res: any = await fetchVideoAccountList(); tableData.value = res || [] } catch (e) { console.error(e) }
  loading.value = false
}

const loadAccounts = async () => {
  try { const res: any = await fetchVideoAccountAccounts(); accounts.value = res || [] } catch (e) { console.error(e) }
}

const queryAccounts = (qs: string, cb: any) => {
  cb(accounts.value.filter((i) => i.toLowerCase().includes(qs.toLowerCase())).map((i) => ({ value: i })))
}
...
const handleAdd = () => {
  ...
  formData.value = { accountName: '', recordDate: new Date().toISOString().split('T')[0], playCount: 0, likeCount: 0, commentCount: 0, shareCount: 0 }
  dialogVisible.value = true
}

const handleEdit = (row: any) => {
  isEdit.value = true; editId.value = row.id
  formData.value = { accountName: row.accountName || '', recordDate: row.recordDate || new Date().toISOString().split('T')[0], playCount: row.playCount || 0, likeCount: row.likeCount || 0, commentCount: row.commentCount || 0, shareCount: row.shareCount || 0 }
  dialogVisible.value = true
}
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) { await fetchUpdateVideoAccount(editId.value, submitData); ElMessage.success('更新成功') }
    else { await fetchAddVideoAccount(submitData); ElMessage.success('添加成功') }
    dialogVisible.value = false; loadData()
  } catch (e) { console.error(e) }
}

const handleDelete = async (id: number) => {
  try { await fetchDeleteVideoAccount(id); ElMessage.success('删除成功'); loadData() } catch (e) { console.error(e) }
}

const handleBatchDelete = async () => {
  try { await fetchBatchDeleteVideoAccount(selectedIds.value); ElMessage.success('批量删除成功'); selectedIds.value = []; loadData() } catch (e) { console.error(e) }
}
